...
pub fn create_ext4_rootfs(
    ...
    let mut file = std::fs::File::create(output_blk)
        .context("Failed to create rootfs block file")?;
    ...
    if let Some(mkfs) = crate::find_on_path("mkfs.ext4") {
        ...
        if let Ok(st) = status {
            if st.success() && cfg!(target_os = "linux") {
